...
overlap=wav.wavelist[ik,ispin,bands,0:2*npw].conj()@wav.wavelist[ik,ispin,bands,0:2*npw].T\
             +proj.conj()@np.kron(np.eye(2),paw.Tij)@proj.T
SIGMA = np.array(
            [
                [
                    np.einsum("ik,kj->ij",
                              wav.wavelist[ik,0,bands,i*npw:(i+1)*npw].conj(), wav.wavelist[ik,0,bands,j*npw:(j+1)*npw].T)
                    for j in (0, 1)
                ] for i in (0, 1)
            ])
